...
実際のデータを使用してアプリケーションを使用するためには、このプロジェクトをcloneして`./src/data/`フォルダ内のデータを変更してください。データの形式は別途クラスを定義しているのでそちらを参考にしてください。以下に各ファイルに格納されている情報とそのデータ型(クラス)についてまとめました。詳しくは以下の各ファイルを参照してください。

**変更するファイル一覧**
+ ./data/in-memory-data.service.ts
+ ./data/mock-data.ts
+ ./data/mock-item.ts
+ ./data/mock-record.ts
+ ./data/mosk-team.ts
...
Ex.`{ sp: 101, itemId: 1, a: 2.34, b: 0.3 },`
...
Ex.`{ sp: 101, min:  6.00, max:  8.00, step: 0.10 },`
